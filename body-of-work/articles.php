<?php
/**
 * Articles Page - Display all article content
 * Dedicated page for Naomi's thoughtful articles
 */
...
$pageTitle = 'Articles — Naomi Wendot';
$metaDescription = 'Read Naomi Wendot\'s thoughtful reflections on life, faith, and personal growth.';
...
if ($search) {
    ...
    $articlesCategoryId = null;
    foreach ($categories as $cat) {
        if ($cat['slug'] === 'articles') {
            $articlesCategoryId = (int)$cat['id'];
            break;
        }
    }
    $result = getAllPosts($search, $articlesCategoryId, $page, $perPage);
} else {
    // Standard category fetch with pagination
    $result = getLatestPosts(null, 'articles', $page, $perPage);
}
...
<body class="font-century bg-cream">
    ...
    <section class="relative h-[380px] flex items-center justify-center overflow-hidden">
        ...
        <div class="absolute inset-0">
            <img 
                src="https://images.unsplash.com/photo-1471107340929-a87cd0f5b5f3?w=1600&q=80" 
                alt="Writing and reflection" 
                class="w-full h-full object-cover"
            >
            <!-- Teal-plum gradient overlay with reduced opacity -->
            <div class="absolute inset-0 bg-gradient-to-br from-teal-900 via-plum to-purple-900 opacity-45"></div>
        </div>
        ...
        <div class="relative z-20 text-center px-4 max-w-4xl">
            ...
            <div class="text-sm mb-6">
                ...
                <span class="text-cream text-opacity-60 mx-2">›</span>
                <span class="text-cream font-bold font-century">Articles</span>
            </div>
    ...
    <section class="bg-cream pt-14 pb-6 px-4">
        <div class="max-w-7xl mx-auto text-center">
            <p class="section-label mb-2">In-Depth Reflections</p>
            <p class="text-charcoal text-sm max-w-2xl mx-auto">
                Longer-form pieces exploring faith, personal growth, and the journey of life with intentionality.
            </p>
            ...
            <form method="get" action="" class="max-w-xl mx-auto">
                ...
                <?php if ($search): ?>
                    <div class="text-center mt-3">
                        ...
                        <a href="articles.php" class="ml-2 text-sm text-gold hover:underline">Clear search</a>
                    </div>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="max-w-7xl mx-auto">
            <?php if (!empty($articles)): ?>
                ...
                <div class="mb-6 text-center">
                    <p class="text-sm text-gray-600">
                        Showing <?php echo count($articles); ?> of <?php echo $totalPosts; ?> article<?php echo $totalPosts !== 1 ? 's' : ''; ?>
                            ...
                            <?php if ($isHandwritten || $article['featured_image']): ?>
                                <div class="blog-card-img">
                                    <a href="<?php echo $postUrl; ?>">
                                        <img 
                                            src="<?php echo basePath() . ($article['handwritten_image'] ?? $article['featured_image']); ?>" 
            ...
            <?php else: ?>
                ...
                <div class="text-center py-20">
                    ...
                    <?php if ($search): ?>
                        <p class="text-gray-500 text-lg mb-2">No articles found for "<?php echo htmlspecialchars($search); ?>"</p>
                        <a href="articles.php" class="text-gold hover:underline">View all articles</a>
                    <?php else: ?>
                        <p class="text-gray-500 text-lg mb-2">No articles available yet</p>
                        <p class="text-gray-400 text-sm">Check back soon for thoughtful reflections!</p>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
